...
variable "dns_resolver_subnet_name" {
  type        = string
  default     = "snet-dns-resolver"
  description = "Name of the subnet for the DNS Private Resolver"
  
  validation {
    condition     = can(regex("^[a-zA-Z0-9-_]+$", var.dns_resolver_subnet_name))
    error_message = "Subnet name can only contain alphanumeric characters, hyphens, and underscores."
  }
}

variable "dns_resolver_subnet_address_prefix" {
  type        = string
  default     = "10.0.1.0/28"
  description = "Address prefix for the DNS resolver subnet (must be at least /28)"
  
  validation {
    condition     = can(cidrhost(var.dns_resolver_subnet_address_prefix, 0))
